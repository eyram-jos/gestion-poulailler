...
class AuthRepo {
  ...
  Future<UserModel> signup(String email, String password, String name) async {
    ...
    final um = UserModel(id: user.uid, email: user.email ?? email, name: name);
  ...
  Future<UserModel> login(String email, String password) async {
    ...
    if (data == null) {
      final fallback = UserModel(id: user.uid, email: user.email ?? email, name: '');
      await fs.collection('users').doc(user.uid).set(fallback.toMap());
      return fallback;
    }
